...
- Визуальный редактор предметов контейнера (карточки с иконками предметов).
- Библиотека предметов с категориями и поиском (`shortname`, RU/EN, категория).
- Drag-and-drop: добавить предмет в контейнер, удалить в корзину, менять порядок предметов.
- Точечный флаг на правило контейнера: `Блокировать бонус лута привилегий` (для отключения рангового множителя лута из PrivilegeSystem).
...
- Визуальный редактор подтягивает иконки предметов автоматически по `shortname`:
  - `https://cdn.rusthelp.com/images/public/<shortname>.png`
- Если иконка недоступна, показывается встроенная fallback-иконка.
