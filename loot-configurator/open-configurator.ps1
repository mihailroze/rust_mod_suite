...
param(
    ...
    [int]$Port = 18765,
    [string]$TargetConfigPath = "C:\rust\server\oxide\config\ContainerLootManager.json",
    [string]$PrivilegeConfigPath = "C:\rust\server\oxide\config\PrivilegeSystem.json"
)

$ErrorActionPreference = "Stop"

$serverScript = Join-Path -Path (Join-Path -Path (Split-Path -Path $PSScriptRoot -Parent) -ChildPath "privilege-configurator") -ChildPath "local_server.py"
...
$openUrl = "$baseUrl/loot-configurator/index.html"
...
if (-not (Test-ConfiguratorServer -Url $healthUrl)) {
    ...
    $args = @(
        ...
        "--target-loot-config", $TargetConfigPath
    )
